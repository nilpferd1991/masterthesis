...
In addition to the hit efficiency, also the hit purity plays an important role for the later physics analyses as wrong hits in a track can reduce the resolution. As can be seen, the hit purity as on a rather high level (above 94 \%) and drops as expected for low momentum particles. In comparison to trasan however, the hit purity should be increased further.
...
\begin{description}
  ...
  \item[CDC\-Legendre\-Tracking] Running the axial Legendre track finder with the quad tree search is done in this part as described also in chapter~\ref{chapter-theory}. It uses the full wire hit set (except the background hits). It has also some post-processing steps included directly after the search, which were analyzed and improved in this thesis. \todo{describe?}
...
Figure~\ref{fig-quad-tree-hits} shows the same event as in figure~\ref{fig-quad-tree-segments}, but this time using another way to transform the segments to the Legendre space: each facet in the segment is transformed to a sinogram according to the usual function. \todo{description}

\begin{figure}
  \centering
  \includegraphics[width=0.7\linewidth]{figures/workflow/quad_tree_segments2.png}
  \todo{picture}
  \caption{TODO}
...
How to do it: BDT with trained variables. First results.