...
\begin{figure}
  ...
  \caption{Simplified event for testing the segment quad tree with the used segments colored with MC information. Note that not all segments are used in the picture to increase readability in the Legendre space plots (but are used for calculating the figures of merit).}
...
\begin{table}
  \caption{Figures of merit on simplified events of the quad tree implementations with segments (for axial hits only) in comparison to the axial Legendre track finder. As expected, the quad trees with segments perform worse.}
  \begin{tabular}{lcccc} \toprule
